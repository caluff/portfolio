...
export type PixelatedCanvasProps = {
  ...
  /** Normalized focal point used when positioning a covered or contained image. */
  ...
  React.useEffect(() => {
    ...
    const compute = () => {
      ...
      if (objectFit === "cover") {
        ...
        dx = Math.floor(
          (displayWidth - dw) * Math.max(0, Math.min(1, objectPosition.x)),
        );
        dy = Math.floor(
          (displayHeight - dh) * Math.max(0, Math.min(1, objectPosition.y)),
        );
